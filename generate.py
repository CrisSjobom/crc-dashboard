...
GROUPS = [
    {'num':1,'cls':'g1','icon':'\U0001f680','members':[
        ...
        {'display':"Jo\u00e3o Sant'Anna",'key':"Jo\u00e3o Sant'Anna",'ini':'JO','na':True,'dest':True},
    ]},
    ...
    {'num':3,'cls':'g3','icon':'\U0001f525','members':[
        ...
        {'display':'Isabela Ara\u00fajo','key':'Isabela Ara\u00fajo','ini':'IA','na':True},
    ]},
    ...
    {'num':6,'cls':'g6','icon':'\U0001f4a0','members':[
        ...
        {'display':'William Bartos','key':'William Bartos','ini':'WB','na':True},
...
def fetch_issues():
    ...
    r = requests.post(
        ...
        headers=h, auth=auth,
        json={'jql': 'project = "' + PROJECT + '" AND assignee is not EMPTY ORDER BY key ASC',
...
def ticket_html(t):
    ...
    parts.append('<span class="ticket-key">')
    parts.append('<a href="' + t['url'] + '" target="_blank">' + t['key'] + '</a>')
    parts.append('</span>')
...
def member_html(mem, mi, cls):
    ...
    if dest:
        ...
        star = ' <span>&#11088;</span>'
        badge = '<span class="badge badge-joao">N&atilde;o Atuou</span>'
        member_cls = 'member joao-highlight'
    elif na:
        av = '<div class="avatar">' + ini + '</div>'
        star = ''
        badge = '<span class="badge b-nao-atuou">N&atilde;o Atuou</span>'
        member_cls = 'member'
    else:
        av = '<div class="avatar">' + ini + '</div>'
        star = ''
    ...
    if na or dest:
        tick = '<div class="tickets"><div class="ticket"><span class="ticket-key">&mdash;</span><span class="ticket-summary">Nenhum caso atribu&iacute;do</span>' + badge + '</div></div>'
    else:
        ts = mi.get(key, [])
        if ts:
            tick = '<div class="tickets">' + ''.join(ticket_html(t) for t in ts) + '</div>'
        else:
            tick = '<div class="no-ticket">Nenhum caso atribu&iacute;do ainda</div>'

    return '<div class="' + member_cls + '">' + name_row + tick + '</div>'

def group_html(g, mi):
    members_out = ''.join(member_html(m, mi, g['cls']) for m in g['members'])
    pills = []
    ...
    if g_nao:  pills.append('<span class="stat-pill" style="background:#4a1a1a;color:#f85149">&#10007; ' + str(g_nao) + ' n&atilde;o avan&ccedil;ou</span>')
    if g_ag:   pills.append('<span class="stat-pill" style="background:#1a2f4a;color:#79c0ff">&#9203; ' + str(g_ag) + ' ag. dev</span>')
    if g_pend: pills.append('<span class="stat-pill" style="background:#3a1f1f;color:#ffa657">&#9888; ' + str(g_pend) + ' pend&ecirc;ncia</span>')
    ...
    header = '<div class="group-header"><h2>' + g['icon'] + ' GRUPO ' + str(g['num']) + '</h2><span class="group-xp">' + str(len(g['members'])) + ' membros &middot; ' + str(tc) + ' tickets</span></div>'
...
def generate_html(issues, mi):
    ...
    return (
        ...
        '<title>CRC Dashboard &mdash; Voltando &agrave;s Origens</title>'
        '<style>' + CSS + '</style></head><body>'
        '<header>'
        '<h1>&#x1F3CD;&#xFE0F; &nbsp;CRC &mdash; Voltando &agrave;s Origens &nbsp;&#x1F3CD;&#xFE0F;</h1>'
        '<p>Acompanhamento de casos por grupo &middot; Projeto Centraliza&ccedil;&atilde;o - Reclama&ccedil;&otilde;es - Clientes</p>'
        ...
        '<div class="meta-badge">N&atilde;o avan&ccedil;ou <span style="color:#f85149">' + str(nao) + '</span></div>'
        '<div class="meta-badge">Ag. Dev <span style="color:#79c0ff">' + str(ag) + '</span></div>'
        '<div class="meta-badge">N&atilde;o iniciado <span style="color:#e3b341">' + str(ni) + '</span></div>'
        '<div class="meta-badge">Pend&ecirc;ncias <span style="color:#ffa657">' + str(pd) + '</span></div>'
        '<div class="live-badge"><span class="live-dot"></span>Atualizado ' + now + '</div>'
        '</div></header>'
        '<div class="grid">' + cards + '</div>'
        '<div class="footer">&#x1F504; Atualizado automaticamente a cada 30 min via GitHub Actions &middot; ' + now + '</div>'
        '</body></html>'
    )

def push_file(path, content, msg):
    h = {'Authorization': 'token ' + GH_PAT, 'Accept': 'application/vnd.github.v3+json', 'Content-Type': 'application/json'}
    url = 'https://api.github.com/repos/' + REPO + '/contents/' + path
    r = requests.get(url, headers=h)
    sha = r.json().get('sha') if r.status_code == 200 else None
    enc = base64.b64encode(content.encode('utf-8')).decode()
    payload = {'message': msg, 'content': enc, 'branch': 'main'}
    if sha:
        payload['sha'] = sha
